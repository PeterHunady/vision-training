...
const dotBlinkDuration = ref(0.7) // seconds default
...
function computeSpawnPosition() {
  ...
  const spawnRadius = Math.max(0, cy - half) * 0.85   // 85% safety radius
...
function startBlinking() {
  // ensure we toggle every dotBlinkDuration seconds
  stopBlinking()
...
function stopBlinking() {
  ...
  // leave showDot true for next time by default
  showDot.value = true
}
...
function startTraining() {
  ...
  // start blinking based on loaded setting
  startBlinking()

  // wait 2 seconds before first spawn
  respawnTimer = setTimeout(spawnCircle, 2000)
...
onMounted(() => {
  // auto-start training when page mounts (keeps original behavior)
  startTraining()
})
...
.dot {
  ...
  /* no CSS animation controlling visibility — JS toggles showDot */
}
